...
export async function getRegionByName(
  regionName: string
): Promise<NamedAPIResourceWithId[]> {
  let endpoints = [];
  ...
  const pokedex = axios.spread<AxiosResponse<Pokedex>, Pokedex[]>(
    (...responses) => {
      return responses.map((response) => response.data);
    }
  )(responses);

  const pokemonSpecies: NamedAPIResource[] = pokedex.reduce(
    (acc: NamedAPIResource[], item: Pokedex) => {
      return acc.concat(
